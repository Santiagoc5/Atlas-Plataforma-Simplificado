...
const UploadPreview = ({ file, url, text }) => (
  <div>
    <img
      ...
      style={{ width: 60, height: 60, objectFit: "cover", borderRadius: 8, margin: "0 auto 8px", display: "block" }}
      alt=""
    />
...
const ProductForm = ({ token, product, categorias, onClose, onSaved }) => {
  ...
  const validate = () => {
    ...
    if (form.precio_oferta !== "" && form.precio_oferta !== null) {
      ...
      if (!isNaN(precioRegular) && !isNaN(precioOferta) && precioOferta >= precioRegular) {
  ...
  return (
    ...
    >
      ...
      <div className="grid2">
        <InputField label="Nombre"   error={errors.nombre} value={form.nombre} onChange={set("nombre")} placeholder="Pastillas de freno XC200" />
        <InputField label="SKU"      error={errors.sku}    value={form.sku}    onChange={set("sku")}    placeholder="PF-XC-200" />
      </div>
      <div className="grid3">
        <InputField label="Precio regular" error={errors.precio}        value={form.precio}        onChange={set("precio")}        type="number" placeholder="59.99" />
        <InputField label="Precio oferta"  error={errors.precio_oferta} value={form.precio_oferta} onChange={set("precio_oferta")} type="number" placeholder="49.99" />
        <InputField label="Stock"          error={errors.stock}         value={form.stock}         onChange={set("stock")}         type="number" placeholder="100" />
      </div>
      ...
      <div className="grid2">
        ...
        <InputField label="Peso (kg)"    error={errors.peso}        value={form.peso}       onChange={set("peso")}       type="number" placeholder="0.5" />
        <InputField label="Dimensiones"  error={errors.dimensiones} value={form.dimensiones} onChange={set("dimensiones")} placeholder="10x5x3 cm" />
      </div>
      ...
      <div style={{
        ...
        <div
          style={{ position: "relative", width: 44, height: 24, cursor: "pointer", flexShrink: 0 }}
          onClick={() => set("en_oferta")(!form.en_oferta)}
        >
            ...
            {mainImg
              ...
              : product?.imagen
                ...
                : <div>
                    ...
                    <p style={{ color: "var(--text2)", fontSize: 13, marginTop: 8 }}>Subir imagen principal</p>
                    <p style={{ color: "var(--text3)", fontSize: 12 }}>PNG, JPG, WEBP</p>
                  </div>
            }
          </div>
        </Field>
            ...
            <div className="gallery" style={{ marginTop: 8 }}>
              {extraImgs.map((f, i) => (
                <div key={i} className="gallery-thumb" style={{ border: "1px solid rgba(230,0,0,0.3)" }}>
                  ...
                  <button className="gallery-remove" onClick={() => setExtraImgs(extraImgs.filter((_, j) => j !== i))}>
                    <X size={9} />
                  </button>
                </div>
              ))}
